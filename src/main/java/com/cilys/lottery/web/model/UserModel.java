...
public class UserModel extends BaseModel<UserModel> {
    ...
    public static List<UserModel> queryAll(){
        return dao.find("select * from " + SQLParam.T_USER);
    }
     ...
     * @return
     */
    public static boolean updateUserLeftMoney(UserModel um){
        if (um == null){
            return false;
        }
        return um.update();
    ...
    /**
     * 更新账户余额，需增加事务，同步各个数据，updateUserLeftMoney方法需去掉
     * @param um
     * @return
     * @throws Exception
     */
    public static boolean updateUserMoney(UserModel um) throws Exception {
        return um.update();
    }
}